...
currentTeams.on("change", (newValue, oldValue) => {
  if (!oldValue || oldValue[0].name !== newValue[0].name) {
    subtleFade([boardAname], newValue[0].name, oldValue);
  }
  if (!oldValue || oldValue[1].name !== newValue[1].name) {
    subtleFade([boardBname], newValue[1].name, oldValue);
  }
  if (!oldValue || oldValue[0].score !== newValue[0].score) {
    subtleFade([boardAscore], newValue[0].score, oldValue);
  }
  if (!oldValue || oldValue[1].score !== newValue[1].score) {
    subtleFade([boardBscore], newValue[1].score, oldValue);
  }
  nodecg.readReplicant("loadedData", "cq-overlay-controls", (value) => {
    if (!oldValue || oldValue[0].name !== newValue[0].name) {
      subtleFade(
        [rostersA],
        value.teamlist[newValue[0].name].map((name) => Roster(name)).join(""),
        oldValue,
        "<div></div>"
      );
    }
    if (!oldValue || oldValue[1].name !== newValue[1].name) {
      subtleFade(
        [rostersB],
        value.teamlist[newValue[1].name].map((name) => Roster(name)).join(""),
        oldValue,
        "<div></div>"
      );
    }
...
const socials = ["otd.ink/discord", "twitter.com/Off_The_Dial"];
let socialIndex = 0;
anime({
  duration: 15000,
  loop: true,
  loopBegin: (anim) => {
    ...
    subtleFade(
      [socialText],
      socials[socialIndex % socials.length],
      socialIndex > 1
    );
    const icons = [Discord(), Twitter()];
    subtleFade(
      [socialIcon],
      icons[socialIndex % icons.length],
      socialIndex > 1,
      "<svg></svg>"
    );
  },
...
currentFlavorText.on("change", (newValue, oldValue) => {
  subtleFade([brbFlavor], newValue, oldValue);
});
